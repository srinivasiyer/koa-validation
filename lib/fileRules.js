...
class FileRules {
    ...
    * size(field, file, deleteOnFail, args, message){
        ...
        if(args && Array.isArray(args) && args.length >= 2 && args.length % 2 === 0){
            ...
            if(!min && !max){
                this.validator.error(field, 'rule', 'size', 'Max or Min properties have not been provided');
                success = false;
            }else{
                if(max && file.size <= max){
                    this.validator.error(field, 'rule', 'size', message || 'The file size exceeds the max size provided');
                    success = false;
                }

                if(min && file.size >= min){
                    this.validator.error(field, 'rule', 'size', message || 'The file size is lower than the min size stated');
                    success = false;
                }
            }
        }else{
            this.validator.error(field, 'rule', 'size', 'Invalid Arguments provided for the size arguement')
            success = false;
        }
    ...
    * extension(field, file, deleteOnFail, args, message){
        ...
        if(!success){
            this.validator.error(field, 'rule', 'extension', message || 'The extension mentioned did not match with the one of the file');
    ...
    * mime(field, file, deleteOnFail, args, message){
        ...
        if(!success){
            this.validator.error(field, 'rule', 'mime', message || 'The mime type mentioned does not match with the file type');
            if(deleteOnFail){
                if(file.path && yield fs.exists(file.path)){
                    yield fs.remove(file.path);
                }
    ...
    * image(field, file, deleteOnFail, message) {
        ...
        if(!success){
            this.validator.error(field, 'rule', 'image', message || 'The file type posted is not an image');
    ...
    * name(field, file, deleteOnFail, filename, message){
        ...
        if(!success){
            this.validator.error(field, 'rule', 'name', message || 'The filename did not match with the posted file');
